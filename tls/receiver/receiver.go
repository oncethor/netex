...
func main() {
	...
	tlsConfig := netex.GenerateTLSConfig([]string{"byte-transfer-test"})
	addrPort := fmt.Sprintf(":%v", port)
	l, err := tls.Listen("tcp", addrPort, tlsConfig)
	if err != nil {
		log.Fatal(err)
	}
	defer l.Close()
	log.Println("Accepting SECURE connections on " + addrPort)
	for i := 1; ; i++ {
		conn, err := l.Accept()
		if err != nil {
			log.Fatal(err)
